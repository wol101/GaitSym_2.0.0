...
pgd::Quaternion Marker::GetWorldQuaternion()
{
    if (mBody)
    {
        const dReal *bodyRotation = dBodyGetQuaternion(mBody);
...
void Marker::Dump()
{
    ...
    if (m_DumpStream)
    {
        ...
        *m_DumpStream << gSimulation->GetTime() << "\t" << p.x << "\t" << p.y << "\t" << p.z <<
...
void Marker::Draw()
{
    ...
    FacetedSphere sphere(mRadius, kLevels);
    sphere.SetColour(m_Colour);
    sphere.SetDisplayPosition(p.x, p.y, p.z);
    sphere.SetDisplayRotation(qq);
    sphere.Draw();
}
#endif
