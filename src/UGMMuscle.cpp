...
void UGMMuscle::SetFibreComposition(dReal fastTwitchFraction)
{
...
void UGMMuscle::SetMuscleGeometry(dReal pcsa, dReal optimumFibreLength, dReal relativeWidth, dReal tendonLength,
...
dReal UGMMuscle::GetESE()
{
    dReal energy;
    dReal delLen = m_Strap->GetLength() - m_lce - m_tendonlength;
...
dReal UGMMuscle::GetEPE()
{
    dReal energy;
    dReal delLen = m_lce - m_lceopt;
    if (delLen < 0) return 0;
...
void UGMMuscle::SetModellingConstants(dReal specificTension, dReal relativeContractionVelocity, dReal muscleDensity)
{
...
void UGMMuscle::SetStim(dReal stim, dReal timeIncrement)
{
    ...
    //dReal lce = m_Length - m_tendonLength;
    dReal c0 = -1 / SQUARE(m_width);
    dReal relLen = m_lce / m_lceopt;
    ...
    dReal Afact = pow(m_act, -0.3);
    dReal arel = Afact * m_arel;
    if (m_lce >= m_lceopt)
        arel = arel * m_fiso;
    ...
    dReal speedLimit = m_vmaxft * m_lceopt;
    if (m_vce < -speedLimit)
    {
...
dReal UGMMuscle::GetMetabolicPower()
{
    // this function converted directly from fortran
    // altering variable names as required

    dReal lcerel = m_lce/m_lceopt;
    dReal vcerel = m_vce/m_lceopt;
...
void UGMMuscle::Dump()
{
    ...
    if (m_DumpStream)
    {
        *m_DumpStream << gSimulation->GetTime() << "\t" << m_stim << "\t" << m_act << "\t" << GetSPE() << "\t" <<
